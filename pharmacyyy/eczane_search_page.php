   ...
   <div class="box-container">
      ...
      <?php
        if (isset($_POST['search_btn'])) {
            ...
            $select_products = mysqli_query($conn, "SELECT products.*, eczanestok.stok_miktarı
FROM products
LEFT JOIN eczanestok ON products.ilaç_id = eczanestok.ilaç_id AND eczanestok.eczane_id = (SELECT eczane_id FROM eczane WHERE users = '$user_id')
WHERE products.name LIKE '%{$search_box}%'") or die('query failed');
                    
            if (mysqli_num_rows($select_products) > 0) {
               while ($fetch_products = mysqli_fetch_assoc($select_products)) {
         ?>
               <form action="" method="POST" class="box">
                  <a href="eczane_view_page.php?product_id=<?php echo $fetch_products['ilaç_id']; ?>" class="fas fa-eye"></a>
                  <div class="price"><?php echo $fetch_products['price']; ?> TL</div>
                  <img src="uploaded_img/<?php echo $fetch_products['image']; ?>" alt="" class="image">
                  <div class="name"><?php echo $fetch_products['name']; ?></div>
                  <div class="stok">Stok Miktarı: <?php echo $fetch_products['stok_miktarı']; ?></div>
